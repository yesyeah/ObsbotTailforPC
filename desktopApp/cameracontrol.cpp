...
bool CameraControl::isInited(){
    return hasInitAlready;
}
...
bool CameraControl::WifiStatusSet(bool is_ap_mode, std::string wifi, std::string pwd){
    std::string url = cameraConctrlAddr;
    std::string res;
    json send_msg;
...
std::vector<std::string> CameraControl::DeviceSearch(){
    ...
    std::cout<<localIP<<" is local ip"<<std::endl;
    ...
    for (int i = 0; i < 127; i++){
        ...
        if (pingHandle->Ping(testIP.c_str(), &reply, 50)){
...
bool CameraControl::CameraUVCModeCtl(bool on){
    remoHandle.UsbModeSet(on);
    return true;
}

bool CameraControl::CameraUVCModeGet(){
    if (remoHandle.UsbModeGet() == UsbMode::NET){
        return true;
    }else{
        return false;
    }
...
int CameraControl::CameraWhiteBalanceGet(){
    return remoHandle.WhiteBalanceGet();
    //return true;
}

bool CameraControl::CameraWhiteBalanceSet(int value){
    remoHandle.WhiteBalanceSet(value);
    return true;
}

int CameraControl::CameraISOGet(){
    return remoHandle.IsoCurGet();
}

bool CameraControl::CameraISOSet(int value){
    remoHandle.IsoMSet(value);
    return true;
}

int CameraControl::CameraExposeGet(){
    return remoHandle.ExposureCurGet();
}

bool CameraControl::CameraExposeSet(int value){
    remoHandle.ExposureModeSet(value);
    return true;
}

bool CameraControl::CameraPoweroff(){
    remoHandle.PowerOff();
    return true;
}
